...
export async function performOAuthFlow(
  ...
  try {
    ...
    const { code, receivedState } = await startLocalServerForCallback(
      config.redirectUri
    )
    ...
    if (state !== receivedState) {
      throw createUserError(
        "OAuth state mismatch. Authentication may have been tampered with",
        {
          category: ErrorCategory.AUTHENTICATION,
          resolution:
            "Try the authentication process again. If the issue persists, contact support.",
        }
      )
    }
 ...
 */
async function startLocalServerForCallback(
  redirectUri: string
