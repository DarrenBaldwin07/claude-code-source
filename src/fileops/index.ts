...
class FileOperationsManager {
    ...
    try {
      ...
      if (!stats.isDirectory()) {
        throw createUserError(
          `Workspace path is not a directory: ${this.workspacePath}`,
          {
  ...
  async readFile(filePath: string): Promise<FileOperationResult> {
    ...
    try {
      ...
      // Check file size
      const maxSizeBytes =
        this.config.fileOps?.maxReadSizeBytes || 10 * 1024 * 1024 // 10MB default

      if (stats.size > maxSizeBytes) {
        return {
          success: false,
          error: createUserError(
            `File too large to read: ${filePath} (${stats.size} bytes)`,
            {
              category: ErrorCategory.FILE_SYSTEM,
              resolution:
    ...
    try {
      ...
      if (errnoError.code === "ENOENT") {
        return {
          success: false,
          error: createUserError(
            `Directory does not exist: ${path.dirname(filePath)}`,
            {
              category: ErrorCategory.FILE_SYSTEM,
              resolution:
                "Use the createDirs option to create parent directories",
            }
          ),
        }
      }
  ...
  async listDirectory(
    dirPath: string
  ): Promise<FileOperationResult & { files?: string[] }> {
    const absolutePath = this.getAbsolutePath(dirPath)
    ...
    try {
      ...
      if (errnoError.code === "EACCES") {
        return {
          success: false,
          error: createUserError(
            `Permission denied listing directory: ${dirPath}`,
            {
              category: ErrorCategory.FILE_SYSTEM,
              resolution:
 ...
 */
export async function initFileOperations(
  config: any
): Promise<FileOperationsManager> {
