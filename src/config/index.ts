...
function loadConfigFromEnv(): any {
  ...
  if (
    process.env.CLAUDE_TELEMETRY === "0" ||
    process.env.CLAUDE_TELEMETRY === "false"
  ) {
...
export async function loadConfig(options: any = {}): Promise<any> {
    ...
    if (options.config) {
      ...
      if (customConfig) {
        ...
        throw createUserError(
          `Could not load configuration from ${options.config}`,
          {
            category: ErrorCategory.CONFIGURATION,
            resolution:
              "Check that the file exists and is valid JSON or JavaScript",
          }
        )
      }
    }
  ...
  import("../utils/logger.js")
    .then((loggerModule) => {
      if (typeof loggerModule.configureLogger === "function") {
