...
export class AuthManager extends EventEmitter {
  private state: AuthState = AuthStateValues.INITIAL
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (this.currentToken) {
        ...
        if (
          isTokenExpired(this.currentToken, this.config.tokenRefreshThreshold)
        ) {
          ...
          if (this.currentToken.refreshToken) {
            try {
              ...
              logger.warn(
                "Failed to refresh token, will need to re-authenticate"
              )
              this.currentToken = null
              this.setState(AuthStateValues.INITIAL)
            }
          } else {
            logger.warn(
              "No refresh token available, will need to re-authenticate"
            )
            this.currentToken = null
            this.setState(AuthStateValues.INITIAL)
          }
          ...
          this.setState(AuthStateValues.AUTHENTICATED)
      ...
      this.setState(AuthStateValues.FAILED)
  ...
  isAuthenticated(): boolean {
    return this.state === AuthStateValues.AUTHENTICATED && !!this.currentToken
  }
  ...
  async authenticate(method?: AuthMethod): Promise<AuthResult> {
    ...
    const authMethod =
      ...
      (this.config.apiKey ? AuthMethod.API_KEY : AuthMethod.OAUTH)

    logger.info(`Authenticating using ${authMethod} method`)
    this.setState(AuthStateValues.AUTHENTICATING)
  ...
  private async authenticateWithApiKey(): Promise<AuthResult> {
    ...
    return {
      ...
      token,
      state: AuthStateValues.AUTHENTICATED,
    }
  }
  ...
  private async refreshToken(): Promise<void> {
    ...
    this.setState(AuthStateValues.REFRESHING)
  ...
  private setState(newState: AuthState): void {
    if (this.state !== newState) {
      logger.debug(
        `Authentication state changed from ${this.state} to ${newState}`
      )
      this.state = newState
